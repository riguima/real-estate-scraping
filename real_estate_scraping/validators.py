...
def valid_real_estate(real_estate: RealEstate) -> None:
    validation_functions = [
        valid_negotiation_type, valid_city,
        valid_phone_number, valid_address, valid_publication_date,
        valid_ad_url]
...
def valid_negotiation_type(real_estate: RealEstate) -> None:
    if real_estate.negotiation_type not in ['Venda', 'Aluguel']:
        raise exceptions.InvalidNegotiationType('Escolha somente Venda ou Aluguel')
...
def valid_phone_number(real_estate: RealEstate) -> None:
    ...
    if number_length != 11 and number_length != 8:
        raise exceptions.InvalidPhoneNumber('Numero de telefone inválido')


def valid_address(real_estate: RealEstate) -> None:
    regex = re.compile(r'\w+ \d+, \w+')
    if not regex.findall(real_estate.address):
        raise exceptions.InvalidAddress('Endereço inválido')


def valid_publication_date(real_estate: RealEstate) -> None:
    if real_estate.publication_date > datetime.now().date():
        raise exceptions.InvalidPublicationDate('Data de publicação não pode ser uma data futura')
